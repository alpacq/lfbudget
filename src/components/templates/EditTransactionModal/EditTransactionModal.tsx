...
const EditTransactionModal = () => {
  const [isOpen, setIsOpen] = useAtom(editTransactionModalAtom);
  ...
  const [transaction, setTransaction] = useAtom(editTransactionAtom);
  const ctx = api.useContext();

  useEffect(() => {
    setIsReturnable(transaction ? transaction.isReturnable : false);
    setTransactionType(
      transaction
    ...
    setAmount(transaction ? transaction.amount.toString() : "0.00");
    setDate(transaction ? transaction.date : new Date(Date.now()));
    setDescription(transaction ? transaction.description : "");
    setCategory(
      categories.find((cat) => cat.category.id === transaction?.categoryId) ??
        null
    );
  }, [transaction, categories]);

  const { mutate } = api.transactions.edit.useMutation({
    onSuccess: () => {
      setTransaction(null);
      setIsOpen(false);
